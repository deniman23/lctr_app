...
SSH_OPTS=(-i "$DEPLOY_SSH_KEY_FILE" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null)
...
"${SCP[@]}" "$APK_FILE" "$MANIFEST_FILE" deploy-release-on-server.sh \
  "${DEPLOY_SSH_USER}@${DEPLOY_SSH_HOST}:${DEPLOY_REMOTE_DIR}/"

"${SSH[@]}" bash -s "$DEPLOY_REMOTE_DIR" "$LOCATOR_GO_RELEASES_DIR" "$APK_FILE" <<'REMOTE'
set -euo pipefail
PRIMARY="$1"
GO_DIR="$2"
APK="$3"
...
echo "=== all manifest.json on server ==="
find /root /var/www -path '*/releases/manifest.json' 2>/dev/null | sort -u | while read -r f; do
